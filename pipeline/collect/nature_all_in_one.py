...
def scoped_section_nodes(soup: BeautifulSoup, section_id: str | None):
    if not section_id:
        return [soup]
...
def postfetch_article(art_url: str, out: str, max_figs: int, max_empty_figs: int, sleep: float, timeout: float, max_retries: int):
    ...
    if found is None or figure_count <= 0:
        base = Path(out) / aid
        if base.exists():
            shutil.rmtree(base, ignore_errors=True)
        return (aid, -1)
...
def cmd_auto(args):
    ...
    if not getattr(args, "stream", False):
        ...
        ns = argparse.Namespace(
            ...
            max_empty_figs=args.max_empty_figs,
            sort=args.sort,
            sleep=args.sleep,
            timeout=args.timeout,
            max_retries=args.max_retries,
...
def build_parser():
    ...
    s.add_argument("--out", default="data/outputs/search_run")
    ...
    f.add_argument("--out", default="data/outputs/nature_content")
    ...
    sd.add_argument("--out", default="data/outputs/nature_content")
    ...
    pf.add_argument("--out", default="data/outputs/nature_content")
    ...
    au.add_argument("--search-out", default="data/outputs/search_auto")
    au.add_argument("--content-out", default="data/outputs/nature_content")
