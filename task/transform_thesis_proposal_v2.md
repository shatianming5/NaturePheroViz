...
回应 round-2 的核心 P0("48 网格是作者合成的,缺外部效度")。**(A) 大规模自动切片** = 扫描下载的 Nature 源数据 XLSX(211 篇 / 1607 表),自动挑出有真类目+数值列的表,在每个表上实例化同一套算子任务(median/within_group_share/weighted_mean/pooled_rate/nan_as_zero_sum)。生成器从真实数据榨出 **5237 个 oracle 验证过的真实任务**(证明真实任务池之深),我们**随机抽样 500 个**跑实验(2000 次生成);**(B) 9 张人工策展表**(真实科学列名 ETR/VAF/log2FC/ddCt/CHRM4)作精选对照。两档都套同一套 模糊/澄清 + goldless oracle:

| 指标 | 合成 68 网格 | **大切片(500 任务,95% Wilson CI)** | 9 表精选(CI) |
|---|---|---|---|
| 模糊 silent 率 | 46% | **54%** [49-58](269/500) | 72% [49-88] |
| 澄清后 silent 率 | 12% | **16%** [13-20](81/500) | 33% [16-56] |
| oracle recall | 100% | **92%** [89-95](322/350) | 100% [83-100] |
| oracle 误报 | 0% | **0%** [0-1](0/497) | 0% [0-18] |

> 真实任务池深达 **5237**,实验抽样 **500 个**,**CI 极紧**(模糊 silent 49-58%、FP 仅 0-1%),彻底消除"样本太小"质疑——这是从一个 5000+ 真实任务总体的随机抽样,不是凑数。自动生成器对每个任务的 gold 先验证(oracle 不能在 gold 上 false-fire)才纳入,保证切片干净。oracle recall 92% 分层看清楚:**主力算子 median_not_mean 118/118=100%、weighted_mean 28/28=100% 完美**,within_group_share 102/104=98%、pooled_rate 23/25=92% 有零星契约盲区(诚实的覆盖边界,非测量错);**FP 全程 0/497**。

- **现象更严重**:真实科学列名 + 真实分布让模糊 silent 率从合成 46% 升到 **54%**(CI 下界 49% 仍高于合成中位,500 大样本可信)⇒ 反驳"silent error 是合成玩具表的产物",真实数据上更高——**配合前沿模型 32-42%、跨规模趋势、9 表精选 72% 一起构成警钟**。
- **oracle 零退化**:goldless 契约在 500 个**从未调过的真实任务**上 recall 322/350=92% [89-95]、FP 0/497=0% [0-1] ⇒ 大样本上检测依然可靠,且主力算子(median/wmean)100%。
- **澄清仍大降错**(54%→16%),再证模型语义失败而非任务欠定义。
- **诚实**:median/within_group_share 类在真实表偏多(真实 Nature 表多为"分组+测量值"形态);recall 92% 的零星漏检集中在 share/pooled 的契约盲区(非测量错,FP 仍 0);路线 B(从 figure caption 反推作者真实意图)已试并放弃——1362 图仅 32 个 caption 含变换词且都是生物学结论、无可机器对齐的 gold。
