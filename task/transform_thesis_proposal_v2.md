...
回应 round-2 的核心 P0("48 网格是作者合成的,缺外部效度")。**(A) 大规模自动切片** = 扫描下载的 Nature 源数据 XLSX,自动挑出有真类目+数值列的表,在每个表上实例化同一套算子任务(median/within_group_share/weighted_mean/pooled_rate/nan_as_zero_sum),共 **206 个真实任务、跨 17 篇论文**(824 次生成);**(B) 9 张人工策展表**(真实科学列名 ETR/VAF/log2FC/ddCt/CHRM4)作精选对照。两档都套同一套 模糊/澄清 + goldless oracle:

| 指标 | 合成 68 网格 | **大切片(206 任务,95% Wilson CI)** | 9 表精选(CI) |
|---|---|---|---|
| 模糊 silent 率 | 46% | **69%** [62-75](142/206) | 72% [49-88] |
| 澄清后 silent 率 | 12% | **18%** [14-24](38/206) | 33% [16-56] |
| oracle recall | 100% | **94%** [89-97](169/180) | 100% [83-100] |
| oracle 误报 | 0% | **0%** [0-2](0/210) | 0% [0-18] |

> 大切片把样本从 9 表升到 **206 任务**,**CI 极紧**(模糊 silent 62-75%、FP 仅 0-2%),彻底消除"样本太小"质疑。自动生成器对每个任务的 gold 先验证(oracle 不能在 gold 上 false-fire)才纳入,保证切片干净。oracle recall 94% 分层看清楚:**主力算子 median_not_mean 129/129=100%、weighted_mean 27/27=100% 完美**,within_group_share 105/107=98%、pooled_rate 23/25=92% 有零星契约盲区(诚实的覆盖边界,非测量错);**FP 全程 0/210**。

- **现象更严重**:真实科学列名 + 真实分布让模糊 silent 率从合成 46% 升到 **69%**(CI 下界 62% 远高于 0)⇒ 彻底反驳"silent error 是合成玩具表的产物",它在真实数据上严重得多——**这是论文的警钟数字之一**(配合前沿模型 32-42% 与跨规模趋势)。
- **oracle 零退化**:goldless 契约在 206 个**从未调过的真实任务**上 recall 169/180=94% [89-97]、FP 0/210=0% [0-2] ⇒ 大样本上检测依然可靠,且主力算子(median/wmean)100%。
- **澄清仍大降错**(69%→18%),再证模型语义失败而非任务欠定义。
- **诚实**:median/within_group_share 类在真实表偏多(真实 Nature 表多为"分组+测量值"形态);recall 94% 的零星漏检集中在 share/pooled 的契约盲区(非测量错,FP 仍 0);路线 B(从 figure caption 反推作者真实意图)已试并放弃——1362 图仅 32 个 caption 含变换词且都是生物学结论、无可机器对齐的 gold。
