...
| 指标 | 合成 48 网格 | **真实 Nature 切片(95% Wilson CI)** |
|---|---|---|
| 模糊 silent 率 | 46% | **72%** [49-88](13/18) |
| 澄清后 silent 率 | 12% | **33%** [16-56](6/18) |
| oracle recall | 100% | **100%** [83-100](19/19) |
| oracle 误报 | 0% | **0%** [0-18](0/17) |

> **canonical 计数**:真实切片所有数字以此表为准(来自最新 real-slice run:13+6=19 silent / 17 correct,带生成抖动)。master 表(`results_master/master_table.md`)与本表一致。

- **现象更严重**:真实科学列名 + 真实分布让 silent 率从 46% 升到 72%(CI 下界 49% 仍远高于 0)⇒ 彻底反驳"silent error 是合成玩具表的产物"——它在真实数据上更糟。
- **oracle 零退化**:goldless 契约在**从未调过的真实数据**上 recall 19/19=100% [83-100]、FP 0/17=0% [0-18] ⇒ 比"在自家网格上 100%"强得多的迁移证据。
- **澄清仍大降错**(72%→33%),与合成版一致,再证模型语义失败而非任务欠定义。
...
6. **typed 归因准确率(已得,契约硬化后)**:双口径——(a) **归因 recall 26/26 = 100%**:对每个 silent error,真实算子的契约都实质 fire,定位到正确算子语义(对真实算子,缺期望输出列=产出形状错=真 silent,算检出);(b) **cross-fire 8%**(90/1104):把不相关契约套到正确结果上的实质误 fire 率——经 schema 门(params 不匹配 abstain)+ 形状门(缺输出列=不适用=abstain,非误归因)后从 20% 降到 8%。修正在测量层完成,不动 check()/契约内部,故 baseline 的 100% recall 不受影响。
