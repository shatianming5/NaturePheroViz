...
→ **核心主张("已建充分契约的类上 100% recall / 0 FP")在扩到 17 类后依然成立**;整体 80% 是新族覆盖**进行时**拉低的,而非核心能力退化。`zscore` 的典型漏检是"per-group mean(z)≈0"不变量无法区分 `ddof=0` 与 `ddof=1`——两者均满足该不变量,而 LLM 在 clarified 条件下仍倾向 pandas 默认的 `ddof=1`,oracle 不触发(B 复现实测 2 例漏,详见 §5 参数敏感型契约盲区);coverage 表以 `zscore: 25%` 标注,修复需补"组内 z 的 std(ddof=0)≈1"检验。FP 全程 0/190,**覆盖增长不抬误报**这一核心保证不变。
...
- **参数敏感型契约盲区(`zscore_within_group` 已知漏检)**:`c_zscore_within_group` 的不变量"per-group mean(z) ≈ 0"足以区分"全局 z-score vs 组内 z-score"这一最典型滑法,但对**同一语义内的参数变体**失效:LLM 以 `std(ddof=1)`(pandas 默认)替代规范的 `std(ddof=0)` 时,per-group mean(z) 仍为 0,oracle 不触发。已实测确认(B 复现,`results_baseline_b`):两个模型在 `zscore_within_group#1 clarified` 条件下均产出此错误,oracle 全部漏检,贡献 scalability demo 整体 recall 由 100% 降至 87% 的 2 例漏检。此盲区属于 scalability demo"新探索"族,**不影响核心 12 类 100% recall 的主张**;coverage 表以 `zscore: 25%` 诚实标注。修复路径已知(加补验"组内 z 的 std(ddof=0) ≈ 1"),当前版本保守 abstain,不引入对 ddof 约定敏感的强主张。
