...
> 📌 **具体例子**:让 LLM 算"转化率从 `r0=20%` 升到 `r1=30%` 的变化"。
> - 它常输出 **50**(把"变化"算成相对增长 `(30−20)/20×100`)
> - 用户要的是 **10 个百分点**(`(30−20)×100`)
>
> 两个数都"看着合理"、代码都能跑、结果都是个正常数字——但一个对、一个错,**没有任何报错告诉你它选错了**。这就是 silent error。(契约见 `eval/transform_oracle.py:c_pct_point`)
...
> 📌 **具体例子**:一张五柱状图 `revenue_bar`(A–E),把 E 的真值从 **99000 悄悄改成 59400**,绘图代码一行没动,图照样画得漂漂亮亮。MatPlotBench / Plot2Code 这类评测看"代码像不像、图像不像、能不能跑"——**三项全过,却没有任何一项在问'E 这根柱画的数对不对'**。(fixture 见 `eval/silent_error_audit.py:revenue_bar`)
...
> 📌 **具体例子**:接着上面那张 `revenue_bar`,我们在 `ax.bar(cat, rev)` 被调用的**瞬间**截下它真正收到的数组 `rev=[130000, 90000, 110000, 145000, 59400]`,直接和真值表对账 → 一眼命中 **E:应 99000、实 59400**。不靠看图、不靠 VLM 猜——读的是代码喂给 matplotlib 的原始实参。(实现见 `agent/app/services/plot_trace.py`)
...
> 📌 **具体例子(转折的两面)**:
> - **画图很少错**:给干净数据让强模型画标准柱/折线图,20 个真实任务 silent error **≈ 0%**(GPT-4o/Claude 全对)。
> - **变换系统性错**:让 LLM "给每个城市加一列 `share` = 该城市销售占比"(`region=[N,N,S,S]`, `sales=[30,10,20,20]`)。它常算成**占全表总和**(`30/80=0.375`…),而用户要的是**占本区域**(N 区:`30/40=0.75`)。两种结果都在 0–1 之间、都"像占比",但只有一个对。这类错强模型有 **GPT-4o 38% / Claude 25%**。(case 见 `eval/transform_bench.py` within_group_share)
...
> 📌 **具体例子(两条线各一个)**:
> - **Transform 线**(算子契约):"百分点 vs 百分比"——契约规定 `pp` 必须 `=(r1−r0)×100`;模型输出 50 而非 10 点时,契约**不看 gold** 也能判它违约(它把"点"算成了相对 %)。
> - **Viz 线**(PlotTrace):双轴误绑——本该画在右轴的"转化率"系列被接到了左轴。看 PNG 完全正常,但 PlotTrace 按 `ax_id` 分轴,发现该 series 落在了错误的轴上。
...
> 📌 **具体例子**:真实 Nature 的 WSe₂ 结合能图,我们注入"Basal 列 ×0.7"。
> - **SVG 反解析**:对这张真实多列科学图直接 `svg-empty-pred`(根本读不出来),clean 图还误报 14/15、定位率 0%。
> - **执行追踪**:精确指出 `x=0.9:应 −4.1、实 −2.87`。
>
> 评测方拿不到执行现场、只能从渲染图反推,在真实图上就失效了——这就是"为什么必须站在生成方、用执行追踪"。(对照见 `eval/silent_error_audit.py --source nature`)
