...
8. **端到端 NL→params→oracle(已得,闭合"params 哪来"质疑)**:契约不再假定 params 由人给。`eval/transform_intent_infer.py` 从**纯 NL prompt + df schema**(关键词 + dtype 角色 + schema 消歧,无 gold/无 LLM)推断 (op, params)。68 网格上 **op 准确率 100%、param-key 100%、端到端 recall 84% = params-given 上界、FP 0%**(`eval/end2end_infer.py`):自动构造 params **零损失**于人工给定。**三层诚实泛化**(`eval/results_generalize/`):网格 100% 是模板共设计上界;held-out off-lexicon paraphrase 上 regex 仅 **28%**(但 abstain-safe,不乱接);改用**免-key LLM 分类器**(opencode free)同一 paraphrase 集 **83%**——NL→operator 对部署可解,契约本身不变。
9. **外部边界确认 + 真 firing(已得,W2,诚实)**:把推断器套到 **DS-1000 全 275 题**,keyword 覆盖 17% 但 ref-code 精度仅 10% → 真覆盖 2%(`eval/oracle_transfer.py`):自由文本 SO 题正则取参不可靠,**确认"契约需算子 params"结构边界**。**但逐题真 firing 已验**(`eval/w2_firing.py`,opencode free 生成解 + DS-1000 gold 判标签,per-case JSON):pilot 12 题 silent 6、recall 3/6=50%、FP 0/2;scaled 30 题 silent 5、**recall 2/5=40% [CI 12-77]、FP 1/6=17% [3-56]**、crash 19/30——在真实外部 silent 上 goldless oracle 确实 fire(零/低误报),**诚实瓶颈是 free 模型 63% crash 致有效样本少、CI 宽(非 oracle 问题)**;命名 API 模型(~\$1/40 题)放大 N 是最便宜决定性下一步。高覆盖外部证据仍是 Nature §1.3.1 切片(params 已知、recall ~99%)。
